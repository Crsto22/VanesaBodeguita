...
import DeleteCompraDrawer from '../components/Compra/DeleteCompraDrawer';
import { useAuth } from '../context/AuthContext';
import { useCompras } from '../context/ComprasContext';
import ComprasHistorialIcon from '../assets/ComprasHistorial/ComprasHistorial.svg';
...
const ComprasHistorial = () => {
  ...
  const quickAccessOptions = [
    ...
    },
    {
      ...
      color: 'bg-blue-500',
      description: 'Registrar compras y ver historial',
      path: '/compras',
    },
  ];
  ...
  useEffect(() => {
    const cargarComprasIniciales = async () => {
      ...
      try {
        ...
        setError('No se pudieron cargar las compras. Inténtalo de nuevo.');
      }
    };
  ...
  const handleDeleteCompra = (compra, e) => {
    e.stopPropagation();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await eliminarCompra(selectedCompra.id);
      ...
      setError('No se pudo eliminar la compra. Inténtalo de nuevo.');
    } finally {
  ...
  const totalProductos = comprasHistorial.reduce((acc, compra) =>
    acc + compra.productos.reduce((sum, prod) => sum + prod.cantidad, 0), 0
  );
  const totalMonto = comprasHistorial.reduce((acc, compra) => acc + compra.total, 0);
      ...
      <main className="px-3 pb-20 pt-3">
        <div className={`transition-opacity duration-500 ${appear ? 'opacity-100' : 'opacity-0'}`}>
          <div className="relative mb-3 overflow-hidden rounded-3xl bg-gradient-to-br from-[#45923a] to-[#34722c] p-6 text-white shadow-lg">
            <img
              src={ComprasHistorialIcon}
              alt="Compras Historial Icon"
              className="absolute right-0 top-1/2 -translate-y-1/2 w-28 h-28 object-contain z-0"
            />
            <div className="relative flex justify-between items-center">
              <div className="flex flex-col">
                <h1 className="mb-2 text-xl font-bold">Historial de Compras</h1>
                <button
          ...
          {loadingHistorial ? (
            <div className="space-y-3">
              {[...Array(COMPRAS_POR_PAGINA)].map((_, index) => (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-lg font-medium">No se encontraron compras</p>
              <p className="text-gray-500 text-sm mt-1">
                {filtroFecha === 'hoy' ? 'No hay compras registradas hoy' :
                  filtroFecha === 'ayer' ? 'No hay compras registradas ayer' :
                    filtroFecha === 'semana' ? 'No hay compras en los últimos 7 días' :
                      filtroFecha === 'mes' ? 'No hay compras en los últimos 30 días' :
                        filtroFecha === 'personalizada' ? 'No hay compras en la fecha seleccionada' :
                ...
                {comprasHistorial.map((compra) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-gray-500">
                          {formatDateShort(compra.fecha_creacion)}
                        </span>
                        <span className="text-xs text-gray-400">
                          {formatTime(compra.fecha_creacion)}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className={`flex items-center gap-1 px-2 py-1 rounded-full border text-xs font-medium ${getEstadoColor(compra.estado)}`}>
                          <CheckCircle className="w-4 h-4 text-green-600" />
                          <span>{compra.estado.charAt(0).toUpperCase() + compra.estado.slice(1)}</span>
                        </div>
                        <button
                          onClick={(e) => handleDeleteCompra(compra, e)}
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <Package size={16} strokeWidth={2.25} className="text-[#ffa40c] flex-shrink-0" />
                      <span className="font-medium text-gray-900 truncate">{compra.nombre_proveedor}</span>
                    </div>
                    <div className="mb-3">
                      <div className="text-xs text-gray-500 mb-2">Productos:</div>
                      <div className="text-sm text-gray-700 leading-relaxed">
                        {compra.productos.slice(0, 2).map((p, index) => (
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-bold text-green-600 text-lg">S/{compra.total.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="px-4 py-3 border-b border-gray-100">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {comprasHistorial.length} compras mostradas • {COMPRAS_POR_PAGINA} por página
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mt-3 flex items-center justify-center gap-4 text-xs text-gray-500">
                    ...
                    {hayMasPaginas && (
                      <div className="flex items-center gap-1">
                        ...
                        <span>Más compras disponibles</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-4 bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="bg-blue-50 rounded-lg p-4 text-center">
                    ...
                    <div className="text-xs text-blue-600 mt-1">Productos comprados</div>
                  </div>
                  <div className="bg-green-50 rounded-lg p-4 text-center">
                    <div className="text-xl sm:text-lg font-bold text-green-600">S/{totalMonto.toFixed(2)}</div>
                    <div className="text-xs text-green-600 mt-1">Total gastado</div>
                  </div>
                </div>
              </div>
            </>
