...
const ComprasHistorial = () => {
  ...
  const { obtenerTodasCompras, eliminarCompra, compras, loading: comprasLoading } = useCompras();
  ...
  useEffect(() => {
    const fetchCompras = async () => {
      try {
        setLoading(true);
        const comprasData = await obtenerTodasCompras();
  ...
  }, [currentUser, obtenerTodasCompras]);
  ...
  useEffect(() => {
      ...
      switch (filtroFecha) {
        ...
        case 'ayer':
          comprasFiltradas = compras.filter(compra => {
            const fechaCompra = new Date(compra.fecha_creacion);
            fechaCompra.setHours(0, 0, 0, 0);
            return fechaCompra.getTime() === ayer.getTime();
          });
          break;
        case 'semana':
    ...
    filtrarCompras();
  }, [compras, filtroFecha, fechaPersonalizada]);
  ...
  // Calcular totales
  const totalProductos = comprasFiltradas.reduce((acc, compra) =>
    acc + compra.productos.reduce((sum, prod) => sum + prod.cantidad, 0), 0
  );
  const totalMonto = comprasFiltradas.reduce((acc, compra) => acc + compra.total, 0);
            ...
            <div className="block space-y-3">
              {comprasFiltradas.map((compra) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Package size={16} strokeWidth={2.25} className="text-[#ffa40c]" />
                    <span className="font-medium text-gray-900">{compra.nombre_proveedor}</span>
                  </div>
                  <div className="mb-3">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                    <div className="flex items-center gap-1">
                      <span className="font-bold text-green-600">S/{compra.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              ))}
