...
import { ArrowLeft, Search, Package, Check, X, Store, Image as ImageIcon, ChevronLeft, ChevronRight, Filter, Eye, EyeOff, Layers, RotateCcw, Ban, RefreshCw, ShoppingBag, ShoppingCart, CreditCard, Users, Truck, Barcode, Settings, FileText, Loader2 } from 'lucide-react';
...
const ConfigTiendaVirtual = () => {
    ...
    const { todosLosProductos, loading, actualizarProducto } = useProducts();
    ...
    const handleRefreshCache = async () => {
        if (window.confirm("¿Estás seguro de resetear el caché de la tienda virtual?")) {
            setIsRefreshingCache(true);
            try {
                await callApi('/cache/refresh', 'POST');
                alert("¡Caché reseteado correctamente!");
            } catch (error) {
                alert("Error al resetear el caché.");
            } finally {
    ...
    useEffect(() => { setCurrentPage(1); }, [searchTerm, filterMode, filterStatus]);
    ...
    const handleTogglePrecioWeb = async (product) => {
        ...
        try {
            await actualizarProducto(product.id, { ...product, mostrar_precio_web: !product.mostrar_precio_web });
        } catch (error) {
    ...
    const handleStockAction = async (product) => {
        ...
        try {
            if (currentStock > 0) {
                ...
                // Must prevent the loading state from hanging if prompt is cancelled
                // Ideally prompt should be before setting loading, but for consistency with async flow:
                // We'll momentarily unset loading to show prompt, or just accept the tiny delay.
                // React updates are batched, so let's do prompt first.
            }
        } catch (error) {
    ...
    // Wrapper for Stock Action to handle Prompt synchronously before setting loading
        ...
        <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
            ...
            <main className="px-3 pb-16 pt-3">
                <div className={`transition-opacity duration-500 ${appear ? 'opacity-100' : 'opacity-0'}`}>
                    ...
                    {/* Header */}
                    <div className="relative mb-6 overflow-hidden rounded-3xl bg-gradient-to-br from-[#06b6d4] to-[#0e7490] p-6 text-white shadow-lg">
                    ...
                    {/* Filters */}
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center gap-4 mb-6 sticky top-20 z-20">
