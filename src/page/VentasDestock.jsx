...
const VentasDestock = () => {
    ...
    const [alertaComprasActiva, setAlertaComprasActiva] = useState(false);
        ...
        try {
            ...
            const ventaData = {
                ...
                productos: selectedProducts.map((p) => ({
                    producto_ref: p.id,
                    nombre: p.nombre,
                    cantidad: p.tipo_unidad === 'kilogramo' ? 1 : p.cantidad,
                    precio_unitario: p.tipo_unidad === 'kilogramo' ? parseFloat(p.precio) : parseFloat(p.precio_unitario || p.precio),
                ...
                ...(clienteSeleccionado && { mandar_alerta: alertaComprasActiva }),
            };
    ...
    const handleSelectCliente = (cliente) => {
        ...
        setAlertaComprasActiva(cliente.alertas_compras_whatsapp || false);
        setDrawerClientesOpen(false);
                    ...
                    <div className="p-4 bg-gradient-to-r from-[#45923a] to-[#3a7d30] flex-shrink-0">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 text-white flex-wrap">
                                ...
                                {/* Badge de mandar alerta de compras */}
                                {clienteSeleccionado && !clienteSeleccionado.enviar_whatsapp && (
                                    <span className="bg-blue-500 backdrop-blur-sm text-white px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap flex items-center gap-1.5">
                                        <Bell size={12} />
                                        Mandar alerta
                                        <input
                                            type="checkbox"
                                            checked={alertaComprasActiva}
                                            onChange={(e) => setAlertaComprasActiva(e.target.checked)}
