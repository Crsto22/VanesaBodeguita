...
import { ShoppingCart, Trash2, Pencil, CreditCard, Users, History, Barcode, Package, User, PlusCircle, ScanBarcode, X, Milk, Minus, Plus, Check, AlertTriangle } from 'lucide-react';
...
const Ventas = () => {
  ...
  // Refocus input after any click on the page, if no drawers/modals are open
  useEffect(() => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('ventaEnProgreso', JSON.stringify({ clienteSeleccionado, selectedProductos }));
    } catch (err) {
  ...
  // Update selected client data if it changes
  useEffect(() => {
    if (clienteSeleccionado && !clientesLoading) {
      ...
      } else if (clienteActual.nombre !== clienteSeleccionado.nombre || clienteActual.telefono !== clienteSeleccionado.telefono) {
  ...
  useEffect(() => {
    const handleKeyDown = async (e) => {
      if (e.key !== 'Enter' || !barcodeValueRef.current || isProcessingRef.current) {
        return;
      }
      if (drawerEscanearOpen || drawerClientesOpen || drawerProductosOpen || drawerConfirmarOpen || drawerEditarPrecioOpen || priceModalOpen) {
        return;
      }
      ...
      const codeToProcess = barcodeValueRef.current;
      setBarcodeInput('');

      try {
        if (!/^\d+$/.test(codeToProcess)) {
          setToast({ message: 'Código de barras inválido', type: 'error', visible: true });
          return;
        }

        const foundProduct = await obtenerProductoPorCodigoBarrasDirecto(codeToProcess);
        if (foundProduct) {
  ...
  // Refocus input when drawers close
  useEffect(() => {
  ...
  const handleSelectProducto = (producto) => {
    setSelectedProductos((prev) => {
      const existingProductIndex = prev.findIndex(
        (p) => p.id === producto.id && p.precio_unitario === producto.precio_unitario
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
            ...
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full mx-auto overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="p-6">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={() => { setPriceModalOpen(false); barcodeInputRef.current?.focus({ preventScroll: true }); }} className="p-2 rounded-full hover:bg-gray-100"><X className="h-5 w-5 text-gray-500" /></button>
                </div>
                <div className="mb-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button onClick={() => { handleSelectPrecio(selectedProduct.precio); barcodeInputRef.current?.focus({ preventScroll: true }); }} className="flex-1 p-3 rounded-xl border border-gray-200 hover:border-[#45923a] hover:bg-[#f9fdf8] transition-all focus:outline-none focus:ring-2 focus:ring-[#45923a] group relative">
                  ...
                  <button onClick={() => { handleSelectPrecio(parseFloat(selectedProduct.precio_alternativo)); barcodeInputRef.current?.focus({ preventScroll: true }); }} className="flex-1 p-3 rounded-xl border border-gray-200 hover:border-[#ffa40c] hover:bg-[#fff8e6] transition-all focus:outline-none focus:ring-2 focus:ring-[#ffa40c] group relative">
                    <div className="flex flex-col items-center text-center">
      ...
      <main className="pt-3 px-2 sm:px-4 pb-28 flex flex-col h-full">
        <input ref={barcodeInputRef} type="text" value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)} className="absolute opacity-0 pointer-events-none" inputMode="none" autoComplete="off" />
            ...
            <div className="flex flex-row justify-center gap-2 sm:gap-6">
              <div className="relative">
                <button className={`flex btn rounded-full flex-row items-center justify-center p-3 border-none text-white shadow-md transition-all ${clienteSeleccionado ? 'bg-[#ffa40c] hover:bg-[#e69500] pr-10' : 'bg-[#ffa40c] hover:bg-[#e69500]'}`} onClick={() => setDrawerClientesOpen(true)} disabled={clientesLoading}>
                  ...
                  <span className="text-sm truncate max-w-[150px]">{clientesLoading ? 'Cargando clientes...' : clienteSeleccionado ? clienteSeleccionado.nombre : 'Cliente Genérico'}</span>
                </button>
                ...
                >
                  ...
                  >
                    ...
                    <span className="font-medium">
                      Este cliente tiene una deuda de <strong>S/ {deudaTotalClienteSeleccionado.toFixed(2)}</strong>
                    </span>
                  </div>
                </motion.div>
              )}
                    ...
                    <AnimatePresence>
                      {selectedProductos.map((producto, index) => (
                        <motion.div
                          ...
                          transition={{ duration: 0.3, ease: "easeOut" }}
                          className="bg-white rounded-2xl p-4 shadow-lg border border-gray-100/50 flex flex-col"
                        >
                                ...
                                {producto.tipo_unidad === 'kilogramo' ? (
                                  <>
                                    <div className="flex items-center gap-3">
                                      ...
                                      <button onClick={() => handleOpenEditarPrecio(index)} className="bg-gradient-to-r from-blue-50 to-blue-100 hover:from-blue-100 hover:to-blue-200 p-3 rounded-xl transition-all active:scale-95" aria-label="Editar precio"><Pencil size={16} className="text-blue-600" /></button>
                                    </div>
                                    <div className="flex gap-2">
                                      {[{ fraction: 0.25, label: '1/4 kg', title: '250g' }, { fraction: 0.5, label: '1/2 kg', title: '500g' }, { fraction: 0.75, label: '3/4 kg', title: '750g' }].map(({ fraction, label, title }) => (
                                        <button key={fraction} onClick={() => handleFractionPrice(index, fraction)} className="px-3 py-2 text-xs font-bold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 rounded-xl shadow-md transition-all active:scale-95" title={title}>{label}</button>
                                      ))}
                                    </div>
                                  </>
                                ) : (
                                  <div className="flex items-center gap-3 flex-wrap">
                                    <div className="flex items-center bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl overflow-hidden shadow-inner">
                                      <button onClick={() => handleUpdateCantidad(index, producto.cantidad - 1)} className="p-3 text-gray-600 hover:bg-gray-200 disabled:opacity-50 transition-colors active:scale-95" disabled={producto.cantidad <= 1}><Minus size={16} /></button>
                                      <span className="px-4 text-sm font-bold text-gray-800 min-w-[40px] text-center">{producto.cantidad}</span>
                                      <button onClick={() => handleUpdateCantidad(index, producto.cantidad + 1)} className="p-3 text-gray-600 hover:bg-gray-200 transition-colors active:scale-95"><Plus size={16} /></button>
                                    </div>
